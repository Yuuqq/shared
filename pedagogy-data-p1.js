...
`;
  ...
  const DATA = {
    ...
    P10: {
      goal: "分清套话密度、句长变异系数和真正的语言模型困惑度，避免把启发式观察当成生成概率。",
      background: "本工具只做两件本地统计：一份固定套话表的命中疏密，以及句子长度的变异系数。它没有语言模型，不算困惑度，也不输出 AI 生成概率。真正的困惑度需要一个语言模型对每个词的预测概率；句长起伏只能说明节奏，不能单独证明作者是谁。",
      refs: [
      ...
      reflect: [
        "你看到的是套话密度和句长起伏，不是困惑度。哪些句子被标出来，只是因为命中了词表？",
        "AI 检测工具有误判的可能吗？如果一篇人类写的文章被误判为 AI 生成，会有什么后果？"
      ],
      module: "第9章 AIGC与新闻写作（篇Ⅲ 实操篇 · Tier 1 实验工具）",
      terms: [
        "<b>套话密度</b>：固定词表的命中次数除以篇幅。不含这些词，不表示文本是人写的。",
        "<b>句长变异系数</b>：句子长度的标准差除以平均句长。句数少于 3 时本工具不给结论。",
        "<b>困惑度 Perplexity</b>：语言模型对下一个词的惊讶程度。本工具没有计算它。",
        "<b>AIGC</b>：AI 生成内容。本页的结果不能当作某段文字由 AI 生成的证据。"
      ],
      exercise: [
        "【教材实验 9.1】目标：看清启发式指标的边界。步骤：准备自己写的 300 字、套话很多的 300 字、以及一段不含套话表的说明文；分别记录套话密度和句长起伏；写明哪一段会被误读成“像 AI”，以及为什么不能把这个结果当成鉴定。产出物：三段记录加 400 字，必须包含一次误判讨论。"
      ]
    },
    P11: {
